...
export async function PATCH(
  ...
  try {
    ...
    console.log('[Bookings ARCHIVE] Type:', type, 'ID:', id)
    ...
    for (const tbl of candidates) {
      console.log(`[Bookings ARCHIVE] Checking table: ${tbl}`)
      const { data, error } = await supabase.from(tbl).select('id, franchise_id').eq('id', id).maybeSingle()
      console.log(`[Bookings ARCHIVE] ${tbl} result:`, { found: !!data, error: error?.message })
      if (!error && data) {
        foundTable = tbl
        existing = data as any
        console.log(`[Bookings ARCHIVE] Found in ${tbl}:`, existing)
        break
      }
    }

    if (!foundTable || !existing) {
      console.log('[Bookings ARCHIVE] Not found in any candidate table:', candidates)
    ...
    // Archive the booking (soft delete by setting is_archived = true)
    console.log(`[Bookings ARCHIVE] Archiving from ${foundTable}...`)
    ...
    if (error) {
      console.error('[Bookings ARCHIVE] Archive failed:', error)
...
ALTER TABLE bookings ADD COLUMN IF NOT EXISTS is_archived BOOLEAN DEFAULT false NOT NULL;
    ...
    console.log('[Bookings ARCHIVE] Successfully archived')
    return NextResponse.json({ 
      success: true, 
