...
export async function POST(request: NextRequest) {
  try {
    ...
    // First, update the essential status field (this column definitely exists)
    ...
    // Try to update optional return fields (these columns may not exist yet)
    try {
      await supabaseServer
        .from("deliveries")
        .update({
      ...
      console.warn("[Process Return] Optional return fields not updated (columns may not exist):", optionalErr)
    }

    // 6. Update booking status if needed (try/catch for optional fields)
    try {
      await supabaseServer
        .from(bookingTable)
        .update({
