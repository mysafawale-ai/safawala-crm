...
/**
 * PDF Header Component - Company info and logo with modern design
 */
export class PDFHeader {
  static async render(
    ...
    // Add logo on LEFT side if available
    let logoEndX = margin
    if (data.logoBase64) {
      try {
        const logoWidth = 45
        ...
        logoEndX = logoX + logoWidth + 10
        console.log('✅ Logo added to PDF (left side)')
      } catch (e) {
        console.error('❌ Failed to add logo:', e)
      }
    }

    // Company name (RIGHT side of logo, bold, large)
    pdf.setFont('helvetica', 'bold')
    pdf.setFontSize(22)
    pdf.setTextColor(colors.primary[0], colors.primary[1], colors.primary[2])
    pdf.text(data.companyName, logoEndX, y + 8)

    // Company tagline or type (smaller, under name)
    pdf.setFont('helvetica', 'normal')
    pdf.setFontSize(8)
    pdf.setTextColor(colors.lightText[0], colors.lightText[1], colors.lightText[2])
    pdf.text('Premium Event & Wedding Solutions', logoEndX, y + 13)
    
    y += 28

    // Company details box (gray background with border)
    const boxHeight = 28
    pdf.setFillColor(250, 250, 252) // Very light gray
    ...
    if (data.companyAddress) {
      ...
      pdf.text(addressLines, col1X, leftY + 4)
      leftY += 4 + (addressLines.length * 4)
    }
    ...
    if (data.companyPhone) {
      ...
      pdf.setFont('helvetica', 'normal')
      pdf.setTextColor(colors.primary[0], colors.primary[1], colors.primary[2])
      pdf.text(data.companyPhone, col1X + 14, leftY)
      pdf.setTextColor(colors.text[0], colors.text[1], colors.text[2])
      leftY += 5
    }
    ...
    if (data.companyEmail) {
      pdf.setFont('helvetica', 'bold')
      pdf.text('Email:', col2X, rightY)
      pdf.setFont('helvetica', 'normal')
      pdf.setTextColor(colors.primary[0], colors.primary[1], colors.primary[2])
      pdf.text(data.companyEmail, col2X + 13, rightY)
      pdf.setTextColor(colors.text[0], colors.text[1], colors.text[2])
      rightY += 5
    }
    ...
    if (data.companyGST) {
      pdf.setFont('helvetica', 'bold')
      pdf.text('GST No:', col2X, rightY)
      pdf.setFont('helvetica', 'normal')
      pdf.text(data.companyGST, col2X + 16, rightY)
      rightY += 5
    }
    ...
    if (data.companyWebsite) {
      ...
      pdf.setFont('helvetica', 'normal')
      pdf.setTextColor(colors.primary[0], colors.primary[1], colors.primary[2])
      pdf.text(data.companyWebsite, col2X + 18, rightY)
      pdf.setTextColor(colors.text[0], colors.text[1], colors.text[2])
      rightY += 5
    }
