...
class PDFGenerator {
  ...
  private addHeader(): void {
    ...
    if (this.data.company.logo_url) {
      // TODO: Add logo loading functionality
      // For now, add a circular placeholder
    ...
    this.doc.text("📞", xPos, contactY)
    this.doc.text(this.data.company.phone, xPos + 5, contactY)

    // Email
    xPos = this.margin + 60
    this.doc.text("✉", xPos, contactY)
    this.doc.text(this.data.company.email, xPos + 5, contactY)

    // Website
    if (this.data.company.website) {
      xPos = this.pageWidth - this.margin - 40
      this.doc.text("🌐", xPos, contactY)
      this.doc.text(this.data.company.website, xPos + 5, contactY)
    }
  ...
  private addDocumentInfo(): void {
    ...
    yPos += 4
    this.doc.setFont("helvetica", "normal")
    this.doc.text(`📱 ${this.data.customer.phone}`, leftCol + 3, yPos)
    
    if (this.data.customer.email) {
      yPos += 4
      this.doc.text(`✉ ${this.data.customer.email}`, leftCol + 3, yPos)
    }
  ...
  private addNotesAndInstructions(): void {
    ...
    if (this.data.special_instructions) {
      ...
      this.doc.setFont("helvetica", "bold")
      this.doc.text("⚠️ SPECIAL INSTRUCTIONS", this.margin + 3, this.currentY + 5)
